...
class Seq2SeqModel(object):
    ...
    def step(self, session, encoder_inputs, decoder_inputs, target_weights,
        ...
        input_feed = self.get_input_feed(encoder_size, encoder_inputs, decoder_inputs, target_weights)
        # Output feed: depends on whether we do a backward step or not.
        # !!! Need to add hidden state to output_feed here
        if not forward_only:
        ...
        if not forward_only:
            ...
            return outputs[1], outputs[2], self.get_output_logits(session, input_feed, bucket_id)  
        else:
            # No gradient norm, loss, outputs.
            return None, outputs[0], self.get_output_logits(session, input_feed, bucket_id) 
    ...
    def get_input_feed(self, size, encoder_inputs, decoder_inputs, target_weights):
        ...
        last_target = self.decoder_inputs[size].name
        input_feed[last_target] = np.zeros([self.batch_size], dtype=np.int32)
        return input_feed


    def get_output_logits(self, session, input_feed, bucket_id):
        size = self.buckets[bucket_id][0]
        logits = []
        for l in xrange(size):
            logits.append(self.outputs[bucket_id][l])
        return session.run(logits, input_feed)
